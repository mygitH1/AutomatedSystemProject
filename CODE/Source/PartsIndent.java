...
public class PartsIndent extends HttpServlet
{
    ...
    try {
	  ...
	  for(int i = 0; i < vproreq.size();i++)
		{
		   ps.setString(1,((QtyRequired)vproreq.elementAt(i)).getProductId());
           ps.executeQuery();
           rs = ps.getResultSet();
			...
			while (rs.next()) 
			{
				...
				{
					if(pid.equals(((PartRequired)vpartsreq.elementAt(j)).getPartId()))
					{
						qtyreq = ((PartRequired)vpartsreq.elementAt(j)).getQuantity() + (reqr * ((QtyRequired)vproreq.elementAt(i)).getQuantity());
						((PartRequired)vpartsreq.elementAt(j)).setQuantity(qtyreq);
						break;
					}
				}
			}
		...
		while(rs.next())
		{
				...
				for(j=0; j < vpartsreq.size();j++)
				{
					if(pid.equals(((PartRequired)vpartsreq.elementAt(j)).getPartId()))
					{
							...
							qtyreq = Math.abs(((PartRequired)vpartsreq.elementAt(j)).getQuantity() - reqr);
							if(qtyreq < 0)
								((PartRequired)vpartsreq.elementAt(j)).setQuantity(0);
							else
							((PartRequired)vpartsreq.elementAt(j)).setQuantity(qtyreq);
							break;
					}
		...
		for(j=0; j < vpartsreq.size();j++)
		{		
			PartOrderId = ((PartRequired)vpartsreq.elementAt(j)).getPartId() + TDate.toString();
			ps.setString(1, PartOrderId);
			ps.setString(2, ((PartRequired)vpartsreq.elementAt(j)).getPartId());
			ps.setInt(3, ((PartRequired)vpartsreq.elementAt(j)).getQuantity());
	  ...
	  if(rs.next())
	  {
		...
		pw.println("<form name=requoto method=post action='http://peers:8080/servlet/RequestQuotations'>");
